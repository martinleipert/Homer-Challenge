...
class Neck(nn.Module):
    """
    Neck layer that contains lateral convs
    """
    ...
    def __init__(self, in_features, embed_dim):
        ...
        for index, lvl_in_features in enumerate(in_features):
            conv = nn.Conv2d(lvl_in_features, embed_dim, kernel_size=kernel_size, stride=stride)
            lateral_convs.append(conv)
            # lateral_convs.add_module(f"conv_{index}", conv)

        self.add_module("lateral_convs", lateral_convs)
        ...
        for index, lvl_in_features in enumerate(in_features):
            conv = nn.Conv2d(embed_dim, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding)
            fpn_convs.append(conv)
            # fpn_convs.add_module(f"conv_{index}", conv)

        self.add_module("fpn_convs", fpn_convs)
    ...
    def forward(self, x):
        ...
        for index, features in enumerate(x):
            xc = self.lateral_convs[index](features)
            xc = self.fpn_convs[index](xc)
            x_new[str(index)] = xc
...
class FocalNet(nn.Module):
    ...
    def __init__(self,
        ...
        # for i_layer in out_indices:
        #    layer = map_layer(num_features[i_layer], embed_dim*2)

        self.add_module("neck", Neck(self.num_features, embed_dim))
    ...
    def forward(self, x):
        ...
        # Calculate the lateral features
        new_outs = self.neck(outs)
        toc = time.time()
